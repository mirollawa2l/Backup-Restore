#!/bin/bash

# Setup
chmod +x ./backupd.sh
TEST_BASE_DIR="./backup_test_$(date +%s)"
SOURCE_DIR="$TEST_BASE_DIR/source"
BACKUP_DIR="$TEST_BASE_DIR/backups"
mkdir -p "$SOURCE_DIR" "$BACKUP_DIR"

echo "File 1" > "$SOURCE_DIR/file1.txt"
echo "File 2" > "$SOURCE_DIR/file2.txt"
mkdir -p "$SOURCE_DIR/subdir"
echo "File 3" > "$SOURCE_DIR/subdir/file3.txt"

# Start the backup script in the background
./backupd.sh "$SOURCE_DIR" "$BACKUP_DIR" 5 3 &
PID=$!

# Allow the script to run for a short duration
sleep 10

# Kill the backup script
kill "$PID"
wait "$PID" 2>/dev/null

# Test backup content verification
backup_path=$(ls -1d "$BACKUP_DIR"/* | head -1)

if [ -f "$backup_path/file1.txt" ] && \
   [ -f "$backup_path/file2.txt" ] && \
   [ -f "$backup_path/subdir/file3.txt" ]; then
  echo "Test Passed: Backup Content Verification"
  echo "SCORE: 2"
else
  echo "Test Failed: Backup Content Verification"
  echo "SCORE: 0"
  exit 1
fi

# Teardown
rm -rf "$TEST_BASE_DIR"